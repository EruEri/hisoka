...
let rec confirm_choice: type a. continue_on_wrong_input:a input_behavior -> ?case_insensible:bool -> yes:char -> no:char -> prompt:string -> unit -> a  = fun ~(continue_on_wrong_input) ?(case_insensible = true) ~yes ~no ~prompt () ->
  let string_transform = if case_insensible then String.lowercase_ascii else Fun.id in
  let () = Printf.printf "%s [%c/%c]: " 
  prompt
  yes
  no
  in
