...
module Display_Cmd = struct
  ...
  let files_term =
    Arg.(
      value & pos_all string [] & info [] ~docv:"FILES" ~doc:"Files to display"
    )
  ...
  let cmd_term run =
    ...
    in
    Term.(
      const combine $ pixel_mode $ Common.stragtegy_group_term
  ...
  let run cmd =
    ...
    let manager = Manager.Manager.decrypt ~key:encrypted_key () in
    let items_list =
      Manager.Manager.list_name_data ~strategy ~key:encrypted_key ~groups
        manager
    in
    let items_list =
      match files with
      | [] ->
          items_list
      | files ->
          items_list |> List.filter (fun (s, _) -> List.mem s files)
    in
    let () =
      Cbindings.Display.hisoka_show items_list (List.length items_list)
        pixel_mode ()
    in
    ()
...
module Chafa_Test = struct
  type test_cmd = { file : string }

  let file_term =
    Arg.(
      required
      & pos 0 (some file) None
      & info ~doc:"Path to the picture file" []
    )
  ...
  let cmd_man = [ `S Manpage.s_description; `P "Test Chafa output" ]

  let cmd run =
    let info = Cmd.info "chafa" ~doc:cmd_doc ~man:cmd_man in
    Cmd.v info (cmd_term run)
